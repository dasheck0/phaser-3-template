...
export class TimeScalePanel extends BaseObject {
  ...
  create(): void {
    ...
    this.timeScaleText = this.scene.add.text(0, 0, this.currentScale(opts).toString(), { fontSize });
  ...
  decreaseTimeScale(): void {
    const opts = this.options as TimeScalePanelOptions;
    this.currentIndex = (this.currentIndex - 1 + opts.timeScales.length) % opts.timeScales.length;
    this.applyCurrentTimeScale(opts);
  }
