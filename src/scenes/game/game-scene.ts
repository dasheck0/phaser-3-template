...
import type { Platform } from "@/prefabs/game/platform";
...
export class GameScene extends BaseScene {
	...
	private async loadFallbackConfig(): Promise<void> {
		const config: SceneConfig = {
			...
			prefabs: [
				...
				{
					type: "Platform",
					layout: { space: "full", x: 600, y: 400 },
					properties: { width: 200, height: 32 },
				},
				{
					type: "Platform",
					layout: { space: "full", x: 150, y: 300 },
					properties: { width: 200, height: 32 },
				},
				{
					type: "Player",
	...
	private setupUI(): void {
		...
		this.input.keyboard?.on("keydown-PLUS", () => {
			gameStore.actions.setMasterVolume(
				gameStore.getState().masterVolume + 0.1,
			);
		});
