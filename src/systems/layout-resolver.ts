...
const createSafeRect = (
	visibleWorldRect: LayoutRect,
	margin?: SceneSafeMargin,
): LayoutRect => {
...
const getAxisTokenValue = (
	...
	if (axis === "x") {
		...
		if (token === "center" || token === "centerX" || token === "x")
			return (rect.left + rect.right) / 2;
	}
	if (token === "top") return rect.top;
	if (token === "bottom") return rect.bottom;
	if (token === "center" || token === "centerY" || token === "y")
		return (rect.top + rect.bottom) / 2;
	throw new Error(
		`Unsupported layout anchor token "${token}" for axis "${axis}"`,
	);
};

const ANCHOR_EXPRESSION_RE = /^([a-zA-Z]+)\s*(([+-])\s*(\d+(?:\.\d+)?))?$/;

const parseAnchorValue = (
	value: string | number,
	rect: LayoutRect,
	axis: "x" | "y",
): number => {
...
const getLayoutRect = (
	...
	if (space === "safe") {
		return createSafeRect(
			input.visibleWorldRect,
			input.sceneLayout?.safeMargin,
		);
	}
	return fullRect;
};
...
export const resolvePrefabDefinitions = (
	...
	definitions.map((definition) => resolvePrefabDefinition(definition, input));
